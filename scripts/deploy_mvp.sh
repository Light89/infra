...
env SSH_AUTH_SOCK="" op run --account my.1password.com --env-file "$ENV_DIR/.env" -- ansible-playbook -i inventory/dev/hcloud.yml playbooks/site.yml --user ansible --private-key "$PRIVATE_KEY_FILE"
